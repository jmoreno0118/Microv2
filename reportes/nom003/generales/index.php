...
if(isset($_POST['accion']) and $_POST['accion']=='guardar')
{
  ...
  else
  {
    ...
    try
    {
      ...
      $sql='SELECT id FROM maximostbl WHERE descargaen =:descargaen AND uso=:uso AND estudio = "nom003"';
      $s=$pdo->prepare($sql); 
  ...
  if($cantidad === 1)
  {
    formularioParametros('nom003', $id, $muestreoid, $cantidad, "", "", "", "", 1);
  }
  formularioMediciones('nom003', $id, $muestreoid, $cantidad, $mcompuestas, 2);
}
...
if (isset($_GET['accion']) and $_GET['accion']=='capturar')
{
  fijarAccionUrl('capturar');
	...
	$boton = 'guardar';
	iniciarAgua('nom003', 'NOM 003', $id, 'Aguas');
  ...
  else
  {
		$valores = array("empresagiro" => getEGiro($id),
                     ...
                     "signatario" => getSignatario($id),
				             "responsable" => getResponsables($id, '', 'nom003'),
...
if((isset($_POST['accion']) and $_POST['accion']=='editar') OR (isset($_POST['accion']) and $_POST['accion']=='ver') OR (isset($_POST['accion']) AND $_POST['accion'] == 'volver' AND isset($_POST['meds'])))
{
    ...
    catch (PDOException $e)
    {
		...
		$valores = array("empresagiro" => getEGiro($linea["ordenaguaidfk"]),
                 ...
                 "signatario" => getSignatario($linea["ordenaguaidfk"]),
			           "responsable" => getResponsables($linea["ordenaguaidfk"], $id, 'nom003'),
  ...
  iniciarAgua('nom003', 'NOM 003', $_SESSION['ot'], 'Aguas');
...
if((isset($_POST['accion']) AND $_POST['accion'] == 'salvar') OR (isset($_POST['accion']) AND $_POST['accion'] == 'siguiente') OR (isset($_POST['accion']) AND $_POST['accion'] == 'volver' AND isset($_POST['coms'])))
{
  ...
  { /* If cantidad = 1 */
      ...
      if(isset($_POST['regreso']) AND $_POST['regreso'] === '2')
      {
        formularioParametros('nom003', $_POST['id'], $_POST['muestreoid'], intval($_POST['cantidad']), $_POST['idparametro'], json_decode($_POST['valores'], TRUE), json_decode($_POST['parametros'], TRUE), json_decode($_POST['adicionales'], TRUE), 1);
      }
      formularioParametros('nom003', $_POST['id'], $linea["muestreoid"], $cantidad, "", "", "", "", 1);
  } /* If cantidad = 1 */
  else
  { /* Else cantidad != 1 */
    if(isset($_POST['regreso']) AND $_POST['regreso'] === '2')
    {
      $mcompuestas = json_decode($_POST['mcompuestas'], TRUE);
      formularioMediciones('nom003', $id, $_POST['muestreoid'], $cantidad, $mcompuestas, 1);
    }
    else
    {
      try
      ...
      formularioMediciones('nom003', $id, $linea["muestreoid"], $cantidad, "", 1);
    }
  } /* Else cantidad != 1 */
  exit();
}
...
if (isset($_POST['accion']) and $_POST['accion']=='parametros')
{
  ...
  if(isset($_POST['regreso']) AND $_POST['regreso'] === '2')
  {
	  formularioParametros('nom003', $_POST['id'], $_POST['muestreoid'], $_POST['cantidad'], $_POST['idparametro'], json_decode($_POST['valores'],TRUE), json_decode($_POST['parametros'],TRUE), json_decode($_POST['adicionales'],TRUE), $_POST['regreso'], $_POST['accionparam']);
  }else{
  		$cantidad = 1;
...
if (isset($_POST['accion']) and $_POST['accion']=='captura siralab')
{
  ...
  formularioSiralab('nom003', $_POST['muestreoid'], '', '', $cantidad, 0);
}
...
{
  $_SESSION['post'] = $_POST;
  header('Location: http://'.$_SERVER['HTTP_HOST'].'/reportes/nom003/documentos');
  exit();
}
...
/**************************************************************************************************/
verMeds($_SESSION['ot'], 'nom003' , 'NOM 003');
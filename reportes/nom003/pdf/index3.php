...
if(isset($_POST['accion']) AND ($_POST['accion']=='buscar' OR $_POST['accion']=='informe') OR (isset($_GET['ot']) AND isset($_GET['id'])))
{
    ...
    try   
    {
        $sql='SELECT ordenestbl.id, ordenestbl.ot, ordenestbl.fechalta, muestreosaguatbl.id as "muestreoaguaid",
                ...
                WHERE generalesaguatbl.estudio = "nom003"';
        if(isset($_GET['ot']) AND isset($_GET['id'])){
            $where=' AND estudiostbl.nombre="NOM 003" AND ordenestbl.ot = :ot AND ordenestbl.id = :id';
            ...
            $where=' AND estudiostbl.nombre="NOM 003" AND ordenestbl.ot = :ot';
            $s=$pdo->prepare($sql.$where);
...
/**************************************************************************************************/
    ...
    $sql='SELECT max(fechareporte) as "Fecha"
        ...
        INNER JOIN estudiostbl ON ordenestbl.id = estudiostbl.ordenidfk
        WHERE estudiostbl.nombre="NOM 003" AND ordenestbl.ot = :ot ';
    $s=$pdo->prepare($sql);
    ...
    try   
    {
        $sql='SELECT muestreosaguatbl.identificacion
            ...
            INNER JOIN muestreosaguatbl ON generalesaguatbl.id = muestreosaguatbl.generalaguaidfk
            INNER JOIN estudiostbl ON ordenestbl.id = estudiostbl.ordenidfk
            WHERE estudiostbl.nombre="NOM 003" AND generalesaguatbl.estudio = "nom003" 
            AND ordenestbl.ot = :ot GROUP BY identificacion';
        $s=$pdo->prepare($sql);
        $s->bindValue(':ot', isset($_GET['ot']) ? $_GET['ot'] : $_POST['ot']);
        $s->execute();
    ...
    try   
    {
        $sql='SELECT generalesaguatbl.id, generalesaguatbl.numedicion
            ...
            INNER JOIN muestreosaguatbl ON generalesaguatbl.id = muestreosaguatbl.generalaguaidfk
            WHERE  generalesaguatbl.ordenaguaidfk = :id AND generalesaguatbl.estudio = "nom003";';
        $s=$pdo->prepare($sql);
    ...
    foreach ($muestras as $muestreo) {
        try   
        {
            $sql='SELECT generalesaguatbl.numedicion, muestreosaguatbl.fechamuestreo, muestreosaguatbl.identificacion,
                ...
                FROM  generalesaguatbl
                INNER JOIN muestreosaguatbl ON generalesaguatbl.id = muestreosaguatbl.generalaguaidfk
                WHERE  generalesaguatbl.id = :id AND generalesaguatbl.estudio = "nom003";';
            $s=$pdo->prepare($sql);
...
//--------------------------------------------------------------------------------------------------------------------
        try   
        {
            ...
            $sql='SELECT *
                FROM maximostbl
                WHERE id = :id AND estudio = "nom003"';
            $s = $pdo->prepare($sql);
...
//--------------------------------------------------------------------------------------------------------------------
        try   
        {
            ...
            WHERE generalaguaidfk = :id AND tipo = "Croquis" AND generalesaguatbl.estudio = "nom003"';
            $s=$pdo->prepare($sql);
...
function hojaNueva($pdf, $orden, $pagina, $paginas){
    ...
    $pdf->Cell(0, 3, utf8_decode('RESIDUAL DE ACUERDO A LA NOM-003-SEMARNAT-1996'), 0, 1, 'C');
    $pdf->Ln();
...
function parametrosPDF($pdf, $muestra, $parametros, $maximos, $cantidad, $parametros2, $gya, $coliformes, $metodos){
    ...
    $formulario = array("fechareporte","ssuspendidos","dbo","arsenico","cadmio","cianuros",
                        "cobre","cromo","mercurio","niquel","plomo","zinc", "hdehelminto");

    $formulario2 = array("GyA","coliformes","ssuspendidos","dbo",
                         "fosforo","arsenico","cadmio","cianuros","cobre","cromo","mercurio",
...
function croquisPDF($pdf, $cantidad, $croquis, $signatario, $responsable){
    //var_dump($croquis);
    $imagen = $_SERVER['DOCUMENT_ROOT'].'/reportes/nom003/documentos/'.$croquis['nombrearchivado'];

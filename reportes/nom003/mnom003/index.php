...
if(isset($_POST['accion']) and $_POST['accion']=='guardar')
{
  ...
  try   
  {
      ...
      $s->bindValue(':coliformes', strcmp(trim($_POST['coliformes']),'') !== 0 ? $_POST['coliformes'] : '---' );
      ...
      $s->bindValue(':hdehelminto', strcmp(trim($_POST['hdehelminto']),'') !== 0 ? $_POST['hdehelminto'] : '---' );
...
function verMaximos(){
  ...
  try   
  {
    $s=$pdo->prepare('SELECT id, descargaen, uso FROM maximostbl WHERE estudio = "nom003"');
    $s->execute();
    $e = $s->fetchAll();
    foreach ($e as $value) {

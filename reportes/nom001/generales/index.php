...
if(isset($_POST['accion']) and $_POST['accion']=='guardar')
{
  ...
  else
  {
    ...
    try
    {
      ...
      $sql='SELECT id FROM maximostbl WHERE descargaen =:descargaen AND uso=:uso AND estudio = "nom001"';
      $s=$pdo->prepare($sql); 
      $s->bindValue(':descargaen', isset($_POST['descargaen'])? trim($_POST['descargaen']) : "");
      $s->bindValue(':uso', isset($_POST['uso'])? trim($_POST['uso']) : "");
      $s->execute();
      ...
      $s->bindValue(':nom01maximosidfk', trim($nom01maximosidfk['id']) );
      $s->bindValue(':numedicion', intval(trim($_POST['numedicion'])),PDO::PARAM_INT );
      $s->bindValue(':lugarmuestreo', trim($_POST['lugarmuestreo']) );
      $s->bindValue(':descriproceso', trim($_POST['descriproceso']) );
      $s->bindValue(':materiasusadas', trim($_POST['materiasusadas']) );
      $s->bindValue(':tratamiento', trim($_POST['tratamiento']) );
      $s->bindValue(':Caracdescarga', trim($_POST['Caracdescarga']) );
      $s->bindValue(':estrategia', trim($_POST['estrategia']) );
      $s->bindValue(':numuestras', trim($_POST['numuestras']) );
      $s->bindValue(':observaciones', trim($_POST['observaciones']) );
      $s->bindValue(':tipomediciones', trim($_POST['tipomediciones']) );
      $s->bindValue(':tipodescarga', trim($_POST['tipodescarga']) );
      $s->execute();
      $id=$pdo->lastInsertid();
      ...
      $s->bindValue(':fechamuestreo', trim($_POST['fechamuestreo']) );
      $s->bindValue(':identificacion', trim($_POST['identificacion']) );
      $s->bindValue(':temperatura', trim($_POST['temperatura']) );
      $s->bindValue(':caltermometro', trim($_POST['emtermometro']) );
      $s->bindValue(':caltermometro2', 0);
      /*$s->bindValue(':caltermometro', $calibracion1);
      $s->bindValue(':caltermometro2', $calibracion2);*/
      $s->bindValue(':pH', trim($_POST['pH']) );
      $s->bindValue(':conductividad', trim($_POST['conductividad']) );
      ...
      if(isset($_POST['fechamuestreofin']) AND trim($_POST['fechamuestreofin']) !== "")
      {
        ...
        $s->bindValue(':fechamuestreofin', trim($_POST['fechamuestreofin']) );
        $s->execute();
      }
  ...
  if($cantidad === 1)
  {
    formularioParametros('nom001', $id, $muestreoid, $cantidad, "", "", "", "", 1);
  }
  formularioMediciones('nom001', $id, $muestreoid, $cantidad, $mcompuestas, 2);
}
...
if (isset($_GET['accion']) and $_GET['accion']=='capturar')
{
  fijarAccionUrl('capturar');
	...
	$boton = 'guardar';
  iniciarAgua('nom001', 'NOM 001', $id, 'Aguas');
  ...
  else
  {
		$valores = array("empresagiro" => getEGiro($id),
                     ...
                     "signatario" => getSignatario($id),
				             "responsable" => getResponsables($id, '', 'nom001'),
...
if((isset($_POST['accion']) and $_POST['accion']=='editar') OR (isset($_POST['accion']) and $_POST['accion']=='ver') OR (isset($_POST['accion']) AND $_POST['accion'] == 'volver' AND isset($_POST['meds'])))
{
    ...
    catch (PDOException $e)
    {
		...
		$valores = array("empresagiro" => getEGiro($linea["ordenaguaidfk"]),
                 ...
                 "signatario" => getSignatario($linea["ordenaguaidfk"]),
			           "responsable" => getResponsables($linea["ordenaguaidfk"], $id, 'nom001'),
  ...
  iniciarAgua('nom001', 'NOM 001', $_SESSION['ot'], 'Aguas');
...
if((isset($_POST['accion']) AND $_POST['accion'] == 'salvar') OR (isset($_POST['accion']) AND $_POST['accion'] == 'siguiente') OR (isset($_POST['accion']) AND $_POST['accion'] == 'volver' AND isset($_POST['coms'])))
{
  ...
  { /* If cantidad = 1 */
      ...
      if(isset($_POST['regreso']) AND $_POST['regreso'] === '2')
      {
        formularioParametros('nom001', $_POST['id'], $_POST['muestreoid'], intval($_POST['cantidad']), $_POST['idparametro'], json_decode($_POST['valores'], TRUE), json_decode($_POST['parametros'], TRUE), json_decode($_POST['adicionales'], TRUE), 1);
      }
      formularioParametros('nom001', $_POST['id'], $linea["muestreoid"], $cantidad, "", "", "", "", 1);
  } /* If cantidad = 1 */
  else
  { /* Else cantidad != 1 */
    if(isset($_POST['regreso']) AND $_POST['regreso'] === '2')
    {
      $mcompuestas = json_decode($_POST['mcompuestas'], TRUE);
      formularioMediciones('nom001', $id, $_POST['muestreoid'], $cantidad, $mcompuestas, 1);
    }
    else
    {
      try
      ...
      formularioMediciones('nom001', $id, $linea["muestreoid"], $cantidad, "", 1);
    }
  } /* Else cantidad != 1 */
  exit();
}
...
if (isset($_POST['accion']) and $_POST['accion']=='parametros')
{
  ...
  if(isset($_POST['regreso']) AND $_POST['regreso'] === '2')
  {
	  formularioParametros('nom001', $_POST['id'], $_POST['muestreoid'], $_POST['cantidad'], $_POST['idparametro'], json_decode($_POST['valores'],TRUE), json_decode($_POST['parametros'],TRUE), json_decode($_POST['adicionales'],TRUE), $_POST['regreso'], $_POST['accionparam']);
  }else{
  		$cantidad = 1;
  ...
  {
	  formularioParametros('nom001', $_POST['id'], $_POST['muestreoid'], $cantidad, $_POST['idparametro']);
  }
  else
  {
    formularioParametros('nom001', $_POST['id'], $_POST['muestreoid'], $cantidad);
  }
}
...
if (isset($_POST['accion']) and $_POST['accion']=='captura siralab')
{
  ...
  if(isset($_POST['regreso']) AND $_POST['regreso'] === '2')
  {
    formularioSiralab('nom001', $_POST['id'], json_decode($_POST['valores'], TRUE), json_decode($_POST['mcompuestas'], TRUE), $_POST['cantidad'], $_POST['regreso'], $_POST['accion']);
  }
  else
  {
      $cantidad = 1;
  ...
  formularioSiralab('nom001', $_POST['muestreoid'], '', '', $cantidad, 0);
}
  ...
  if(isset($_POST['terminada']))
  {
      try
      {
        $sql='UPDATE estudiostbl SET
          fechafin = CURDATE()
          WHERE ordenidfk = :id AND nombre = "NOM 001"';
        $s=$pdo->prepare($sql);
...
/**************************************************************************************************/
verMeds($_SESSION['ot'], 'nom001' , 'NOM 001');
...
/*var_dump($_POST['mcompuestas']);
	...
	try
	{
		...
		$s->bindValue(':titulo', trim($_POST['titulo']) );
		$s->bindValue(':anexo', trim($_POST['anexo']) );
		$s->bindValue(':rfc', trim($_POST['rfc']) );
		$s->bindValue(':cuenca', trim($_POST['cuenca']) );
		$s->bindValue(':tipoestudio', trim($_POST['tipoestudio']) );
		$s->bindValue(':numerodescarga', trim($_POST['numerodescarga']) );
		$s->bindValue(':region', trim($_POST['region']) );
		$s->bindValue(':procedencia', trim($_POST['procedencia']) );
		$s->bindValue(':lattgrados', trim($_POST['lattgrados']) );
		$s->bindValue(':lattmin', trim($_POST['lattmin']) );
		$s->bindValue(':lattseg', trim($_POST['lattseg']) );
		$s->bindValue(':lontgrados', trim($_POST['lontgrados']) );
		$s->bindValue(':lontmin', trim($_POST['lontmin']) );
		$s->bindValue(':lontseg', trim($_POST['lontseg']) );
		$s->bindValue(':latpgrados', trim($_POST['latpgrados']) );
		$s->bindValue(':latpmin', trim($_POST['latpmin']) );
		$s->bindValue(':latpseg', trim($_POST['latpseg']) );
		$s->bindValue(':lonpgrados', trim($_POST['lonpgrados']) );
		$s->bindValue(':lonpmin', trim($_POST['lonpmin']) );
		$s->bindValue(':lonpseg', trim($_POST['lonpseg']) );
		$s->bindValue(':datumgps', trim($_POST['datumgps']) );
		$s->bindValue(':comentarios', trim($_POST['comentarios']) );
		$s->execute();
		...
		foreach ($_POST['mcompuestas'] as $value) {
	        ...
	        if(isset($value["identificacion"]) AND strcmp(trim($value["identificacion"]), '') !== 0 ){
	        	$s->bindValue(':identificacion', trim($value["identificacion"]) );
	        ...
	        if(isset($value["fechalab"]) AND strcmp(trim($value["fechalab"]), '') !== 0 ){
	        	$s->bindValue(':fecharecepcion', trim($value["fechalab"]) );
	        ...
	        if(isset($value["horalab"]) AND strcmp(trim($value["horalab"]), '') !== 0 ){
	        	$s->bindValue(':horarecepcion', trim($value["horalab"]) );
	        ...
	        if(isset($value["temperatura"]) AND strcmp(trim($value["temperatura"]), '') !== 0 ){
	        	$s->bindValue(':temperatura', trim($value["temperatura"]) );

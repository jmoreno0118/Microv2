...
if(isset($_POST['accion']) and $_POST['accion']=='subir')
{
	...
	if( strcmp($_FILES['archivo']['tmp_name'], '') === 0 ){
		...
		guardarObs($_POST['id'], $_POST['observaciones']);
		$_SESSION['post'] = $_POST;
		header('Location: .');
		exit();
	}
	...
	{
		$mensaje='Hubo un error tratando de subir el archivo.  Favor de revisar la conexión a internet y que el archivo sea menor a 5Mb e intenta de nevo.';
		include $_SERVER['DOCUMENT_ROOT'].'/reportes/includes/error.html.php';
		exit();
	}
...
function datostabla()
{
	...
	if($docs = $s->fetchAll())
	{
		...
		$sort = array('Croquis','Expediente A', 'Expediente B', 'Expediente C','A1',
...
/* Guanrdar documento en la base de datos */
/**************************************************************************************************/
function guardarObs($id='', $observaciones='')
{
  global $pdo;
  try
  {
	  $sql='UPDATE generalesaguatbl SET
			observacionescroquis=:observacionescroquis
			WHERE id=:id';
	  $s=$pdo->prepare($sql);
	  $s->bindValue(':observacionescroquis', $observaciones);
	  $s->bindValue(':id', $id);
	  $s->execute();
  }
  catch(PDOException $e)
...
function getObs($id='')
{
  ...
  try
  {
	  $sql='SELECT observacionescroquis FROM generalesaguatbl
			WHERE id=:id';
	  $s=$pdo->prepare($sql);
	  $s->bindValue(':id', $id);
	  $s->execute();
	  $obs = $s->fetch(PDO::FETCH_ASSOC);
	  return $obs['observacionescroquis'];
  }
  catch(PDOException $e)
  {
  	return '';

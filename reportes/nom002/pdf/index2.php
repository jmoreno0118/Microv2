...
if(isset($_POST['accion']) AND ($_POST['accion']=='buscar' OR $_POST['accion']=='informe') OR (isset($_GET['ot']) AND isset($_GET['id'])))
{
    ...
    try   
    {
        $sql='SELECT ordenestbl.id, ordenestbl.ot, ordenestbl.fechalta, muestreosaguatbl.id as "muestreoaguaid",
                ...
                WHERE generalesaguatbl.estudio = "nom002"';
        if(isset($_GET['ot']) AND isset($_GET['id'])){
            $where=' AND estudiostbl.nombre="NOM 002" AND ordenestbl.ot = :ot AND ordenestbl.id = :id';
            ...
            $where=' AND estudiostbl.nombre="NOM 002" AND ordenestbl.ot = :ot';
            $s=$pdo->prepare($sql.$where);
...
/**************************************************************************************************/
    ...
    $sql='SELECT max(fechareporte) as "Fecha"
        ...
        INNER JOIN estudiostbl ON ordenestbl.id = estudiostbl.ordenidfk
        WHERE estudiostbl.nombre="NOM 002" AND ordenestbl.ot = :ot ';
    $s=$pdo->prepare($sql);
    ...
    try   
    {
        $sql='SELECT muestreosaguatbl.identificacion
            ...
            INNER JOIN muestreosaguatbl ON generalesaguatbl.id = muestreosaguatbl.generalaguaidfk
            INNER JOIN estudiostbl ON ordenestbl.id = estudiostbl.ordenidfk
            WHERE estudiostbl.nombre="NOM 002" AND ordenestbl.ot = :ot GROUP BY identificacion';
        $s=$pdo->prepare($sql);
        $s->bindValue(':ot', isset($_GET['ot']) ? $_GET['ot'] : $_POST['ot']);
        $s->execute();
...
//--------------------------------------------------------------------------------------------------------------------
        try   
        {
            ...
            $sql='SELECT *
                FROM maximostbl
                WHERE id = :id AND estudio = "nom002"';
            $s = $pdo->prepare($sql);
...
//--------------------------------------------------------------------------------------------------------------------
        try   
        {
            $sql='SELECT *
                ...
                WHERE generalaguaidfk = :id AND tipo = "Croquis" AND generalesaguatbl.estudio = "nom002"';
            $s=$pdo->prepare($sql);
...
//--------------------------------------------------------------------------------------------------------------------
    ...
    /**************************************************************************************************/
            ...
            if(count($parametros2)<=0 OR $parametros2 === ""){
                $mensaje='Faltan llenar datos de las mediciones.'.$e;
                include $_SERVER['DOCUMENT_ROOT'].'/reportes/includes/error.html.php';
                exit();
            }

            if($cantidad === 1){
                parametrosPDF($pdf, $muestra, $parametros, $maximos, $cantidad, $parametros2, 0, $metodos);
            }else{
        ...
        /**************************************************************************************************/
            if($cantidad === 1){
                ...
                parametrosPDF($pdf, $muestra, $parametros, $maximos, $cantidad, $parametros2, $promedio, $metodos);
            }
        ...
        /**************************************************************************************************/
            if($cantidad !== 1){
                ...
                for ($i=0; $i < $cantidad; $i++) { 
                  ...
                  $pdf->Cell(30, 5, utf8_decode(number_format(doubleval($imprimirvolalicuota), 0, '', '')), 1, 1, 'C');
                }
...
function hojaNueva($pdf, $orden, $pagina, $paginas){
    ...
    $pdf->Cell(0, 3, utf8_decode('RESIDUAL DE ACUERDO A LA NOM-002-SEMARNAT-1996'), 0, 1, 'C');
    $pdf->Ln();
...
function parametrosPDF($pdf, $muestra, $parametros, $maximos, $cantidad, $parametros2, $gya, $metodos){
    $pdf->SetFont('Arial', 'B', 9);
    ...
    $formulario = array("fechareporte","ssedimentables","ssuspendidos","dbo","arsenico","cadmio","cianuros",
                        "cobre","cromo","mercurio","niquel","plomo","zinc");

    $formulario2 = array("GyA","ssedimentables","ssuspendidos","dbo",
                         "arsenico","cadmio","cianuros","cobre","cromo","mercurio",
                         "niquel","plomo","zinc");
...
function croquisPDF($pdf, $cantidad, $croquis, $signatario, $responsable){
    //var_dump($croquis);
    $imagen = $_SERVER['DOCUMENT_ROOT'].'/reportes/nom002/documentos/'.$croquis['nombrearchivado'];
